...
class OSREvaluation():
    ...
    def __init__(self,test_loader) -> None:
        ...
        self.test_labels = np.array(labels,np.int32)
        
        self.close_samples = self.test_labels >= 0
        self.close_samples_ct = np.sum(self.close_samples)

        self.unknown_samples = self.test_labels < 0  ##改动
        self.unknown_samples_ct = np.sum(self.unknown_samples)  ##改动
    ...
    def open_detection_indexes(self,scores,thresh):
        ...
        if thresh < -99999:
            tidx = np.abs(np.array(tpr) - 0.90).argmin()    ##阈值改动，原先是0.95
    ...
    def open_reco_indexes(self,scores,thresh,rawpredicts):
        ...
        if thresh < -99999:
            fpr, tpr, thresholds  =  metrics.roc_curve(self.close_samples, scores) 
            thresh = thresholds[np.abs(np.array(tpr) - 0.90).argmin()]           ##阈值改动，原先是0.95
            # print(thresh)
        ...
        unknown_acc = np.sum((predicts == self.test_labels) & self.unknown_samples) / self.unknown_samples_ct  ##改动
