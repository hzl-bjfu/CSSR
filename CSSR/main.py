...
def save_everything(subfix = ""):
    ...
    if subfix == "oscr":
        mth.save_model(saving_path + 'model_oscr.pth')
...
def load_everything(subfix = ""):
    ...
    with open(saving_path + "hist.json",'r') as f:   ##改动
        json.load(f)
...
def training_main():
    ...
    global best_acc,best_auroc,best_oscr
...
def overall_testing():
    ...
    # mth.prepare_data_for_PLDA(test_loader)  ##修改，测试
    # os._exit(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--ds', type=str, required=False, default="./exps/cifar10/cross.json",help='dataset setting, choose file from ./exps')  ##数据集标签
    ...
    saving_path = 'D:\\work\\code\\CSSR2\\model'
    ...
    train_loader , test_loader ,classnum = dataset.load_partitioned_dataset(args,args.ds)  ##注意类别数要去dataset里改
    ...
    if not args.test:
        ...
        load_everything("oscr")   ##这里‘oscr’的意思是会保留oscr指标最佳的模型，也可换其它指标
        overall_testing()
