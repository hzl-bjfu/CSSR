...
from .CenterLoss import CenterLoss  ##改动
import math  ##改动
import os
import pickle
...
class Backbone(nn.Module):
    ...
    def __init__(self,config,inchan,avg_output=False):
        super().__init__()
        ...
        if config['backbone'] == 'wideresnet28-2':
            ...
            self.backbone = ResNet(output_dim=config['category_model']['projection_dim'],inchan = inchan,avg_output=False)  ##改动avg,默认False,ture会将特征池化
...
class CSSRModel(nn.Module):
    ...
    def forward(self,x,ycls = None,reqpredauc = False,prepareTest = False,reqfeature = False):
        ...
        if reqfeature:
           return x
        x,xcls_raw = x   ##x.shape = [16, 512, 28, 28]; xcls_raw.shape = [16, 13, 28, 28]
        # print(xcls_raw)  ##打印
        # print(xcls_raw.shape)  ##打印

        def pred_score(xcls):
            score_reduce = lambda x : x.reshape([x.shape[0],-1]).mean(axis = 1)
            x_detach = x.detach()
            probs = self.crt(xcls,prob = True).cpu().numpy()   ##标注，shape是[16,13],是损失

            pred = probs.argmax(axis = 1)  ##标注，pred实例：[3 3 7 3 11 3 10 3 3 3 11 7 7 3 10 7]，shape为16

            max_prob = probs.max(axis = 1)  ##shape为16，一个批次样本最大的损失

            cls_scores = xcls.cpu().numpy()[[i for i in range(pred.shape[0])],pred]   ##标注，shape为[16，28，28]
            ...
            scores = np.stack([score_reduce(eval(self.config['score'])),score_reduce(rep_cspt),score_reduce(rep_gram)],axis = 1)  ##标注，score
            # print(scores)  ##打印
            # print(scores.shape)  ##打印
            return pred,scores
...
class CSSRCriterion(nn.Module):
    ...
    def __init__(self,avg_order,enable_sigma = True):
        ...
        self.center_loss = CenterLoss(num_classes=7, feat_dim=5488)    ##需要改动，类别数目
    ...
    def forward(self,x,y = None,prob = False,pred = False):
        ...
        new_g = x.view(x.size(0), -1)  #改动，不用centerloss的时候注销

        loss = -torch.sum(self.get_onehot_label(y,g.shape[1]) * torch.log(g),dim=1).mean()  ##交叉熵
        cen_loss = self.center_loss(new_g, y).mean()   ##改动，不用centerloss的时候注销
        loss = loss + math.log(cen_loss)*0.3   ##改动，不用centerloss的时候注销
        return loss
...
class CSSRMethod:
    ...
    def prepare_data_for_PLDA(self,test_loader):  ##改动；添加
        print('\n_____prepare data for PLDA_____')
        self.model.eval()
        data_array = np.array([[[]]])
        ...
        with torch.no_grad():
            ...
            # for d in tqdm.tqdm(self.wrap_loader):
                x1 = d[0].cuda(non_blocking = True)   ##图片 [bs,3,224,224]
                gt = d[1].numpy()   ##标签,[bs]
                x = self.model.backbone_cs(x1, feature_only=True)  ##改动，这里改成True了，不会经过AE了
                # x, xcls_raw = x  ##x.shape = [16, 512, 28, 28]=>特征; xcls_raw.shape = [16, 13, 28, 28],=>分类映射，13是类别数。在提取损失时取消注释
                # probs = self.crt(xcls_raw, prob=True)  ##标注，shape是[16,13],是损失，在提取损失时取消注释
                for emb in x:
                    ...
                    try:
                        data_array = np.append(data_array, emb, axis=0)
                    except:
                        data_array = emb
                for label in gt:
        ...
        f = open(os.getcwd() + "/D3D1_512_test.pkl", 'wb')
        pickle.dump([data_array, label_array], f)
        f.close()
                # print(x.shape) ##打印
                # pred,scr,dev = self.model(x1,reqpredauc = True,prepareTest = False)
    ...
    def knownpred_unknwonscore_test(self,test_loader):
        ...
        pred,scores,devs,gts = self.scoring(test_loader)   ##标注，scores的shape是[2108，3]，2108是所有测试样本的个数
        # print(scores.shape)  ##打印
        ...
        S = scores.T
        # my_para = eval(self.config['integrate_score'])  ##添加
        # print(my_para)   ##打印
        # print(my_para.shape)
